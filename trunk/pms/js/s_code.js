...
if(window.location.hostname.indexOf(".co.uk")>-1 || window.location.hostname.indexOf(".com.tr")>-1 || window.location.hostname.indexOf(".co.yu")>-1 || window.location.hostname.indexOf(".com.pl")>-1) s.cookieDomainPeriods="3"
...
s.linkInternalFilters="javascript:,sony.ru,sony.at,sony.be,sony.bg,sony.hr,sony.cz,sony.dk,sony.ee,sony.fi,sony.fr,sony.de,sony.gr,sony.hu,sony.ie,sony.it,sony.lv,sony.lt,sony.mk,sony.md,sony.nl,sony.no,sony.com.pl,sony.pl,sony.pt,sony.ro,sony.co.yu,sony.sk,sony.si,sony.es,sony.se,sony.ch,sony.co.uk,sony.com.tr,sony.ua,sony-europe.com,43.196.158.146" 
s.linkLeaveQueryString=true
s.linkTrackVars="None"
s.linkTrackEvents="None"

/* Form Analysis Config*/
s.formList="customerProfileForm,productRegistrationProfileForm,productRegistrationCodeForm,productRegistrationModelForm"
...
function s_doPlugins(s) {
	...
	s.campaign=s.getQueryParam('campaignId');
	s.setupFormAnalysis();
	
	/* Segmentation for conversion*/
	if(s.prop1){s.eVar5=s.prop1;}
	if(s.prop2){s.eVar6=s.prop2;}
	if(s.prop3){s.eVar7=s.prop3;}
	if(s.prop4){s.eVar8=s.prop4;}
	if(s.prop5){s.eVar9=s.prop5;}


	/*Adding the ArticleView success event*/
	if (s.pageName.indexOf("Article",0) > 0){
	  s.eVar20=s.pageName;
	  s.events=s.apl(s.events,'event10',',');
	}
  ...
  function customDownloadTracker(obj,downloadname,downloadcategory)
{
    ...
    s.prop21=downloadname;
    s.prop22=brand + " : " + downloadcategory;
    s.tl(obj,'o','Custom download');
}
function customSuccessTracker(obj,eventNames,decription)
{
	var s=s_gi(s_account);
	s.events=eventNames;
	s.linkTrackVars='events';
	s.linkTrackEvents=eventNames;
	s.tl(this,'o',decription);
}
function customRssSubscriptionTracker(obj,rssTitle)
{
	var s=s_gi(s_account);
	s.eVar22=rssTitle;
	s.events='event15';
...
function customRedirectSSSTracker(obj,redirectType)
{
	var s=s_gi(s_account);
	s.eVar21=redirectType;
	s.events='event12';
	s.linkTrackVars='eVar21,events,products';
	s.linkTrackEvents='event12';
	s.tl(this,'o','Redirect to SSS');
}

function customRedirectSSSTrackerWithProductName(obj,redirectType, productName)
{
    var s=s_gi(s_account);
	s.eVar21=redirectType;
	s.events='event12';
...
function customRedirectCategoryTracker(obj,categoryName)
{
	var s=s_gi(s_account);
	s.eVar30=categoryName;
	s.linkTrackVars='eVar30';
	s.tl(this,'o','Redirect Category');
}
